...
import { faustLangRegister, docSections, faustDocURL, faustSyntaxURL } from "@bide/monaco-faust";
...
$(async () => {
    ...
    // Export to Bolt DAW trigger
    $("#btn-export-daw").on("click", () => {
        const code = editor.getValue();
        const name = uiEnv.fileManager.mainFileNameWithoutSuffix || "Untitled";
        try {
            if (window.opener && !window.opener.closed) {
                ...
                alert(`Successfully sent DSP "${name}" to Bolt DAW!`);
            } else {
                alert("BIDE is not connected to an active Bolt DAW window. Open BIDE from the DAW Browser to enable export.");
            }
        } catch (e) {
            if (window.opener) {
                window.opener.postMessage({
                ...
                alert(`Sent DSP "${name}" to Bolt DAW!`);
            } else {
                alert("Failed to export: Opener window is unreachable.");
            }
        }
    });
