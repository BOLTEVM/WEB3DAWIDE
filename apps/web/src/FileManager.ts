...
process = ba.pulsen(1, 10000) : pm.djembe(60, 0.3, 0.4, 1) <: dm.freeverb_demo;`);
    ...
    rename(oldName: string, newName: string) {
        // Guard: Sanitize and prevent directory traversal
        const sanitized = newName.replace(/[^a-zA-Z0-9_.]/g, "");
        if (!sanitized || sanitized !== newName) {
            console.error(`Rename from "${oldName}" to "${newName}" rejected: illegal characters.`);
            return false;
        }
    ...
    newFile(fileNameIn?: string, content?: string | Uint8Array) {
        let fileName = fileNameIn ? fileNameIn.replace(/[^a-zA-Z0-9_.]/g, "") : "";
        if (fileNameIn && (!fileName || fileName !== fileNameIn)) {
            console.warn(`Filename "${fileNameIn}" was sanitized to "${fileName}" to prevent directory traversal.`);
        }
