...
class Key2Midi {
    ...
    velMap: number[] = [];
    /**
     * Map to track absolute midi notes for active key strings to guard against octave-shift hangs
     */
    activeKeyNotes: { [key: string]: number } = {};
    /**
    ...
    handleKeyDown = (str: string): void => {
        ...
        if (typeof converted === "number") {
            ...
            this.activeKeyNotes[str] = note; // Guard: record absolute activated note
            this.handler([144, note, this.velocity]);
            return;
        }
        if (converted === "NEXT") {
            this.flush();
    ...
    handleKeyUp = (str: string): void => {
        if (!this.enabled) return;
        const activeNote = this.activeKeyNotes[str];
        if (typeof activeNote !== "undefined") {
            this.velMap[activeNote] = 0;
            this.handler([144, activeNote, 0]);
            delete this.activeKeyNotes[str]; // Clean up state
        }
    }
    /**
     * Mute All
